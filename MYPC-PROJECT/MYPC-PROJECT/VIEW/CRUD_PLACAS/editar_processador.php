...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $socket = filter_input(INPUT_POST, 'socket', FILTER_SANITIZE_STRING);
    $memoria_max = filter_input(INPUT_POST, 'memoria_max', FILTER_SANITIZE_STRING);

    // Prepara e executa a consulta de inserção
    $sql = "INSERT INTO processadores (nome, socket, memoria_max) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('sss', $nome, $socket, $memoria_max);
    $stmt->execute();
    $stmt->close();
}

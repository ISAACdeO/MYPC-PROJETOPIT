...
if ($placa_video_id) {
    // Busque as informações da placa de vídeo no banco de dados
    $stmt = $pdo->prepare("SELECT * FROM placas_video WHERE id = :id");
    $stmt->bindParam(':id', $placa_video_id, PDO::PARAM_INT);
    $stmt->execute();
    $placa_video = $stmt->fetch(PDO::FETCH_ASSOC);
}
?>
